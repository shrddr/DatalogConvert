...
1. Create all the Historian points that will be used in the import
   ...
   - either FactoryTalk Administration Console - right click the application - Add/Discover Historian points,
   - or use `piconfig.exe < add_points.csv` generated by this script. Point descriptions will be empty though.

   In both case, the span values will default to 0-100 for every point, and the compression deviation to 0.2 engineering units. You might want to review these settings to achieve good value compression.

2. Create a new archive, or force an archive shift in SMT.

3. Perform the import: `piconfig.exe < values.csv`
